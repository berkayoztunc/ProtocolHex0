...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		if game_over_label != null and game_over_label.visible:
			if event.physical_keycode == KEY_R:
				...
				return
			if event.physical_keycode == KEY_M or event.physical_keycode == KEY_ESCAPE:
				hide_game_over()
...
func show_game_over(stats: Dictionary = {}) -> void:
	var weapon_name: String = str(stats.get("weapon", "—"))
	...
	if game_over_label != null:
		game_over_label.text = "GAME OVER\nKills: %d\nLevel: %d\nWeapon: %s\n\n[R] Play Again   [M] Main Menu" % [kills, level, weapon_name]
