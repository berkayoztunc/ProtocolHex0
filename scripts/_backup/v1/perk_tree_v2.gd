...
func _build_tree() -> void:
	...
	accent_bar.color = Color(0.2, 0.55, 0.9, 0.35)
	...
	title.add_theme_color_override("font_outline_color", Color(0.1, 0.15, 0.25, 0.95))
	...
	close_hint.add_theme_color_override("font_color", Color(0.4, 0.45, 0.55))
	...
	var points_text: String = "★ Available Perk Points: %d" % _available_points
	...
	points_label.add_theme_color_override("font_outline_color", Color(0.02, 0.03, 0.05, 0.9))
	if _available_points > 0:
		points_label.add_theme_color_override("font_color", Color(0.45, 0.95, 0.55))
	else:
		points_label.add_theme_color_override("font_color", Color(0.45, 0.48, 0.55))
	...
	var cat_colors: Dictionary = {
		...
		1: Color(0.4, 0.8, 0.6),    # ADVANCED - green
		2: Color(0.7, 0.5, 0.9),    # ADVANCED LVL - purple
		3: Color(0.9, 0.7, 0.3),    # UZMANLIK - gold
		4: Color(0.3, 0.85, 0.85),  # PASSIVE WEAPON - cyan
		5: Color(0.3, 0.75, 0.75),  # PASSIVE BOOST - teal
		6: Color(0.9, 0.4, 0.3),    # ACTIVE WEAPON - red
		7: Color(0.85, 0.35, 0.35), # ACTIVE BOOST - dark red
	}
	for row_idx in CATEGORY_LABELS:
		...
		cat_label.add_theme_color_override("font_outline_color", Color(0.02, 0.03, 0.05, 0.8))
...
func _create_perk_node(perk_id: String, perk_data: Dictionary, stacks: int, max_stacks: int, prereqs_met: bool, is_selectable: bool) -> PanelContainer:
	...
	style.border_color = Color(0.2, 0.28, 0.42, 0.9)
	...
	if is_locked:
		...
		stack_label.add_theme_color_override("font_color", Color(0.3, 0.25, 0.3))
		panel.modulate = Color(0.45, 0.45, 0.5, 0.75)
	elif is_maxed:
		...
		if max_stacks > 0:
			...
			stack_label.text = "◇ %d" % next_cost
		stack_label.add_theme_color_override("font_color", Color(0.35, 0.35, 0.4))
	...
	if tex_style != null:
		if is_maxed:
			...
			tex_style.modulate_color = Color(0.3, 0.3, 0.35, 0.6)
	else:
		if is_maxed:
