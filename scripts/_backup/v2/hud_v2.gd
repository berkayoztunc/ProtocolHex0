...
func update_perk_charges(bomb_charges: int, heal_charges: int) -> void:
	bomb_label.text = "Bomb (Q): %d" % bomb_charges
	heal_label.text = "Heal (E): %d" % heal_charges
...
func _style_game_over_panel(hud_scale: float) -> void:
	_style_panel(game_over_panel, Color(0.04, 0.02, 0.06, 0.97), Color(0.8, 0.15, 0.2, 0.95))
	game_over_title.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(48.0, hud_scale, 1, 36.0)))
	game_over_title.add_theme_color_override("font_color", Color(1.0, 0.25, 0.3))
	game_over_title.add_theme_constant_override("outline_size", 4)
	game_over_title.add_theme_color_override("font_outline_color", Color(0.15, 0.02, 0.04, 0.95))
	for stat_lbl in [game_over_kills_label, game_over_level_label, game_over_weapon_label]:
		stat_lbl.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(20.0, hud_scale, 1, 16.0)))
		stat_lbl.add_theme_color_override("font_color", Color(0.8, 0.85, 0.92))
		stat_lbl.add_theme_constant_override("outline_size", 2)
		stat_lbl.add_theme_color_override("font_outline_color", Color(0.02, 0.02, 0.04, 0.9))
	_style_button(game_over_restart_btn)
	_style_button(game_over_menu_btn)
	game_over_restart_btn.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(20.0, hud_scale, 1, 16.0)))
	game_over_menu_btn.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(16.0, hud_scale, 1, 13.0)))
	var viewport_size: Vector2 = _get_viewport_size()
	var panel_w: float = clampf(viewport_size.x * 0.42, 440.0, 680.0)
	var panel_h: float = clampf(viewport_size.y * 0.52, 360.0, 500.0)
...
func update_active_weapons(weapons_data: Array[Dictionary]) -> void:
	...
	var card_width: float = UiTextureUtils.scale_dimension(118.0, card_scale, 2, 84.0)
	var card_padding_x: int = int(UiTextureUtils.scale_dimension(6.0, card_scale, 1, 4.0))
	var card_padding_y: int = int(UiTextureUtils.scale_dimension(3.0, card_scale, 1, 2.0))
	var inner_spacing: int = int(UiTextureUtils.scale_dimension(4.0, card_scale, 1, 3.0))
	...
	var label_font_size: int = int(UiTextureUtils.scale_dimension(16.0, card_scale, 1, 12.0))
	var cooldown_height: float = UiTextureUtils.scale_dimension(4.0, card_scale, 1, 3.0)
	...
	for wdata in weapons_data:
		...
		card_style.bg_color = Color(wcolor.r * 0.15, wcolor.g * 0.15, wcolor.b * 0.15, 0.92)
		card_style.border_width_left = int(UiTextureUtils.scale_dimension(2.0, card_scale, 1, 1.0))
		card_style.border_width_top = card_style.border_width_left
		card_style.border_width_right = card_style.border_width_left
		card_style.border_width_bottom = card_style.border_width_left
		if is_held:
		...
		lbl.text = "⚔ %s" % wname if is_held else wname
		lbl.add_theme_font_size_override("font_size", label_font_size)
		lbl.add_theme_constant_override("outline_size", 1)
		lbl.add_theme_color_override("font_outline_color", Color(0.02, 0.03, 0.04, 0.9))
		if is_held:
			lbl.add_theme_color_override("font_color", wcolor.lightened(0.45))
		elif is_ready:
			lbl.add_theme_color_override("font_color", wcolor.lightened(0.3))
		else:
...
func _apply_ux_style() -> void:
	var viewport_size: Vector2 = _get_viewport_size()
	var hud_scale: float = _get_hud_scale()
	var outer_margin: int = int(UiTextureUtils.scale_dimension(24.0, hud_scale, 2, 16.0))
	...
	hud_margin.add_theme_constant_override("margin_bottom", outer_margin)
	hud_vbox.add_theme_constant_override("separation", int(UiTextureUtils.scale_dimension(4.0, hud_scale, 1, 3.0)))
	stats_row_1.add_theme_constant_override("separation", int(UiTextureUtils.scale_dimension(12.0, hud_scale, 2, 8.0)))
	stats_row_2.add_theme_constant_override("separation", int(UiTextureUtils.scale_dimension(12.0, hud_scale, 2, 8.0)))
	modal_backdrop.color = Color(0.01, 0.02, 0.04, 0.88)
	_style_panel(stats_bg_panel, Color(0.04, 0.06, 0.10, 0.84), Color(0.2, 0.3, 0.45, 0.55))
	...
	for lbl in [level_label, kill_label, bomb_label, heal_label, targeting_label, weapon_label, projectile_label]:
		lbl.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(17.0, hud_scale, 1, 14.0)))
		lbl.add_theme_constant_override("outline_size", 2)
		lbl.add_theme_color_override("font_outline_color", Color(0.02, 0.03, 0.04, 0.9))
		lbl.add_theme_color_override("font_color", Color(0.82, 0.9, 1.0))

	level_label.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(24.0, hud_scale, 1, 20.0)))
	...
	_style_progress_bar(health_bar, Color(0.8, 0.2, 0.25, 1.0))
	_style_progress_bar(xp_bar, Color(0.25, 0.72, 0.96, 1.0))

	_style_panel(controls_panel, Color(0.08, 0.11, 0.16, 0.93), Color(0.28, 0.42, 0.62, 0.95))
	_style_panel(level_up_panel, Color(0.08, 0.1, 0.15, 0.97), Color(0.35, 0.62, 0.92, 0.95))
	_style_panel(confirm_menu_panel, Color(0.08, 0.1, 0.15, 0.97), Color(0.35, 0.62, 0.92, 0.95))
	active_weapons_panel.modulate.a = 0.92
	active_weapons_panel.add_theme_constant_override("separation", int(UiTextureUtils.scale_dimension(8.0, hud_scale, 1, 6.0)))
	controls_margin.add_theme_constant_override("margin_left", int(UiTextureUtils.scale_dimension(16.0, hud_scale, 1, 12.0)))
	controls_margin.add_theme_constant_override("margin_top", int(UiTextureUtils.scale_dimension(12.0, hud_scale, 1, 10.0)))
	controls_margin.add_theme_constant_override("margin_right", int(UiTextureUtils.scale_dimension(16.0, hud_scale, 1, 12.0)))
	controls_margin.add_theme_constant_override("margin_bottom", int(UiTextureUtils.scale_dimension(12.0, hud_scale, 1, 10.0)))
	controls_vbox.add_theme_constant_override("separation", int(UiTextureUtils.scale_dimension(4.0, hud_scale, 1, 3.0)))
	for control_label in controls_panel.find_children("", "Label", true, false):
		if control_label is Label:
			...
			var font_size: int = int(UiTextureUtils.scale_dimension(15.0, hud_scale, 1, 12.0))
			if label_node.name == "TitleLabel":
				font_size = int(UiTextureUtils.scale_dimension(18.0, hud_scale, 1, 14.0))
	...
	_apply_bar_icons()
	_apply_control_hint_icons()
	_apply_top_button_icons()

	level_up_title.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(32.0, hud_scale, 1, 24.0)))
	level_up_title.add_theme_color_override("font_color", Color(0.9, 0.95, 1.0))
	level_up_title.add_theme_constant_override("outline_size", 2)
	level_up_title.add_theme_color_override("font_outline_color", Color(0.02, 0.03, 0.04, 0.9))
	option_button_a.custom_minimum_size.y = UiTextureUtils.scale_dimension(76.0, hud_scale, 2, 64.0)
...
func _layout_hud() -> void:
	...
	var hud_scale: float = _get_hud_scale()
	var outer_margin: float = UiTextureUtils.scale_dimension(24.0, hud_scale, 2, 16.0)
	var sep: float = UiTextureUtils.scale_dimension(4.0, hud_scale, 1, 3.0)
	var level_row_h: float = UiTextureUtils.scale_dimension(26.0, hud_scale, 1, 21.0)
	var body_row_h: float = UiTextureUtils.scale_dimension(18.0, hud_scale, 1, 15.0)
	# 1 level row + 2 bars + 4 body rows (kills, ammo, weapon type, regen) + 7 separators
	var stats_content_h: float = level_row_h + health_bar.custom_minimum_size.y + xp_bar.custom_minimum_size.y + body_row_h * 4.0 + sep * 7.0
	var stats_height: float = float(outer_margin) * 2.0 + stats_content_h + 16.0
	var icon_col_w: float = UiTextureUtils.scale_dimension(float(ConfigService.get_value("visual.hud.icon_size", 28)) + 12.0, hud_scale, 2, 24.0)
	var stats_width: float = maxf(float(outer_margin) + icon_col_w + health_bar.custom_minimum_size.x + float(outer_margin), UiTextureUtils.scale_dimension(360.0, hud_scale, 4, 320.0))
	...
	perk_tree_button.custom_minimum_size.x = UiTextureUtils.scale_dimension(116.0, hud_scale, 2, 104.0)
	projectile_switch_button.custom_minimum_size.x = UiTextureUtils.scale_dimension(140.0, hud_scale, 2, 118.0)
	menu_button.custom_minimum_size.x = UiTextureUtils.scale_dimension(100.0, hud_scale, 2, 90.0)
	var utility_height: float = UiTextureUtils.scale_dimension(38.0, hud_scale, 2, 34.0)
	var top_sep: float = UiTextureUtils.scale_dimension(10.0, hud_scale, 2, 8.0)
	...
	top_right_container.offset_bottom = outer_margin + utility_height

	var weapon_panel_width: float = clampf(viewport_size.x * 0.56, UiTextureUtils.scale_dimension(380.0, hud_scale, 4, 360.0), UiTextureUtils.scale_dimension(860.0, hud_scale, 4, 860.0))
	var weapon_panel_height: float = UiTextureUtils.scale_dimension(78.0, hud_scale, 2, 58.0)
	...
	var controls_width: float = minf(viewport_size.x * 0.28, UiTextureUtils.scale_dimension(360.0, hud_scale, 4, 280.0))
	var controls_height: float = UiTextureUtils.scale_dimension(286.0, hud_scale, 4, 232.0)
	...
	var level_width: float = clampf(viewport_size.x * 0.54, UiTextureUtils.scale_dimension(620.0, hud_scale, 4, 540.0), UiTextureUtils.scale_dimension(880.0, hud_scale, 4, 880.0))
	var level_height: float = clampf(viewport_size.y * 0.58, UiTextureUtils.scale_dimension(420.0, hud_scale, 4, 420.0), UiTextureUtils.scale_dimension(620.0, hud_scale, 4, 620.0))
	...
	var confirm_width: float = clampf(viewport_size.x * 0.34, UiTextureUtils.scale_dimension(420.0, hud_scale, 4, 360.0), UiTextureUtils.scale_dimension(620.0, hud_scale, 4, 620.0))
	var confirm_height: float = clampf(viewport_size.y * 0.30, UiTextureUtils.scale_dimension(240.0, hud_scale, 4, 220.0), UiTextureUtils.scale_dimension(340.0, hud_scale, 4, 340.0))
...
func _style_button(button: Button) -> void:
	...
	normal.border_color = Color(0.29, 0.43, 0.63, 1.0)
	...
	hover.border_color = Color(0.42, 0.64, 0.92, 1.0)
	...
	disabled.border_color = Color(0.2, 0.22, 0.28, 0.9)
...
func _style_utility_button(button: Button) -> void:
	var hud_scale: float = _get_hud_scale()
	button.custom_minimum_size.y = UiTextureUtils.scale_dimension(38.0, hud_scale, 2, 34.0)
	button.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(14.0, hud_scale, 1, 12.0)))
	var util_tex: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/button_utility_normal.png", 20, StyleBoxTexture.AXIS_STRETCH_MODE_TILE_FIT)
	var util_hover_tex: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/button_utility_hover.png", 20, StyleBoxTexture.AXIS_STRETCH_MODE_TILE_FIT)
	if util_tex != null and util_hover_tex != null:
		button.add_theme_stylebox_override("normal", util_tex)
		button.add_theme_stylebox_override("hover", util_hover_tex)
		button.add_theme_stylebox_override("pressed", util_tex)
		button.add_theme_color_override("font_color", Color(0.75, 0.84, 0.94))
		button.add_theme_color_override("font_hover_color", Color(0.92, 0.97, 1.0))
		return
	var normal: StyleBoxFlat = StyleBoxFlat.new()
	normal.bg_color = Color(0.09, 0.12, 0.18, 0.88)
	normal.border_color = Color(0.22, 0.32, 0.48, 0.9)
	normal.border_width_left = 1
	normal.border_width_top = 1
	normal.border_width_right = 1
	normal.border_width_bottom = 1
	...
	var hover: StyleBoxFlat = normal.duplicate()
	hover.bg_color = Color(0.14, 0.20, 0.30, 0.92)
	hover.border_color = Color(0.32, 0.50, 0.72, 1.0)
	button.add_theme_stylebox_override("normal", normal)
	button.add_theme_stylebox_override("hover", hover)
