...
var _active_mode: String = ""  # "burn" | "explosive" | ""
	...
	for wid in _weapon_slots:
		...
		if float(slot.get("active_time", 0.0)) > 0.0:
			...
			if float(slot["active_time"]) <= 0.0:
				...
				var wdef: Dictionary = _get_weapon_def(wid)
				slot["cooldown_time"] = float(wdef.get("active_cooldown_sec", 60.0)) * cooldown_multiplier
				if bool(wdef.get("is_mode", false)):
					# Mode skill expired: clear active mode flag
					if _active_mode == str(wdef.get("mode_type", "")):
						_active_mode = ""
					weapons_changed.emit()
				elif current_held_weapon == wid:
					# Regular weapon expired: revert to plasma
...
func _activate_weapon_slot(weapon_id: String) -> void:
	...
	# Mode skills: overlay on current weapon, don't switch current_held_weapon
	if bool(wdef.get("is_mode", false)):
		# Cancel any other active mode first
		for wid: String in _weapon_slots:
			if wid == weapon_id:
				continue
			var other_def: Dictionary = _get_weapon_def(wid)
			if bool(other_def.get("is_mode", false)) and float(_weapon_slots[wid].get("active_time", 0.0)) > 0.0:
				_weapon_slots[wid]["active_time"] = 0.0
				_active_mode = ""
				_weapon_slots[wid]["cooldown_time"] = float(other_def.get("active_cooldown_sec", 20.0)) * cooldown_multiplier
		_active_mode = str(wdef.get("mode_type", ""))
		slot["active_time"] = float(wdef.get("active_duration_sec", 5.0))
		slot["cooldown_time"] = 0.0
		weapons_changed.emit()
		return
	# Regular weapon slot: temporarily take over from plasma
	if current_held_weapon != "plasma_rifle":
		...
		if not prev_slot.is_empty():
			...
			prev_slot["cooldown_time"] = float(prev_def.get("active_cooldown_sec", 60.0)) * cooldown_multiplier
...
func _handle_utility_inputs() -> void:
	...
	# Activate weapon slots (keys 1-9)
	for _slot_i in range(1, 10):
		var _action_name: String = "weapon_slot_%d" % _slot_i
		if InputMap.has_action(_action_name) and Input.is_action_just_pressed(_action_name):
			_try_activate_slot(_slot_i)
			break
...
func _fire_weapon(weapon_id: String, wdef: Dictionary) -> void:
	...
	var is_cryo_aoe: bool = wdef.get("is_cryo_aoe", false)
	var is_orbit: bool = wdef.get("is_orbit", false)
	var is_dash_skill: bool = wdef.get("is_dash_skill", false)
	...
	# Cryo Field: AOE freeze — stops all nearby enemies
	if is_cryo_aoe:
		_do_cryo_field(total_damage, wdef)
		return
	...
	# Sonic Jumper: dash in movement direction, deal contact damage
	if is_dash_skill:
		_do_sonic_jump(total_damage, wdef)
		return

	# Magnetic Field: collect all XP gems
	if is_magnet_skill:
		magnet_collect_all()
		return

	# Helix Lazer: spinning laser beam hits all enemies in radius
	if is_spin_laser:
...
func _spawn_weapon_bullet(direction: Vector2, damage_amount: int, wdef: Dictionary, bullet_is_crit: bool = false, is_default: bool = false) -> void:
	# Explosive mode uses AOE projectile scene
	var proj_scene: PackedScene = aoe_bullet_scene if _active_mode == "explosive" else _get_projectile_scene()
	var bullet: Node2D = proj_scene.instantiate()
	...
	bullet.electric_bullet_chance = electric_bullet_chance if is_default else 0.0
	bullet.explosive_bullet_chance = explosive_bullet_chance if is_default else 0.0
	_apply_projectile_class_modifiers(bullet, wdef)

	# Active mode overrides
	if _active_mode == "burn":
		bullet.burn_chance = 1.0
	elif _active_mode == "explosive":
		bullet.is_aoe = true
		bullet.aoe_radius = 70.0
...
func _spawn_orbit_bullets(damage_amount: int, wdef: Dictionary) -> void:
	...
	for i in range(count):
		...
		bullet.electric_bullet_chance = electric_bullet_chance
		bullet.explosive_bullet_chance = explosive_bullet_chance
...
func _do_cryo_field(damage_amount: int, wdef: Dictionary) -> void:
	var cryo_radius: float = float(wdef.get("cryo_radius", 260.0))
	var freeze_dur: float = float(wdef.get("freeze_duration", 3.0))
	var dtype: String = str(wdef.get("damage_type", "cryo"))
	_spawn_vfx_ring("res://assets/vfx/vfx_void_explosion_ring.png", global_position, cryo_radius)
	var enemies: Array = get_tree().get_nodes_in_group("enemies")
	for enemy in enemies:
		if global_position.distance_to(enemy.global_position) <= cryo_radius:
			if enemy.has_method("take_damage"):
				enemy.take_damage(damage_amount, dtype)
...
func _do_multi_nearest_fire(damage_amount: int, wdef: Dictionary, is_crit: bool) -> void:
	var count: int = int(wdef.get("projectile_count", 1))
	var enemies: Array = get_tree().get_nodes_in_group("enemies")
	if enemies.is_empty():
		return
	# Sort by distance
	...
	for i in range(mini(count, enemies.size())):
		var dir: Vector2 = (enemies[i].global_position - global_position).normalized()
		_spawn_weapon_bullet(dir, damage_amount, wdef, is_crit, false)
...
func apply_run_state(saved_state: Dictionary) -> void:
	...
	# Fix any duplicate slot_key values that may be in old save data
	_deduplicate_slot_keys()
	_update_weapon_visual()
	level_changed.emit(level)
	health_changed.emit(health, max_health)
	xp_changed.emit(xp, xp_to_next_level)
...
func _next_available_slot_key() -> int:
	var used: Array[int] = []
	for wid: String in _weapon_slots:
		var k: int = int(_weapon_slots[wid].get("slot_key", 0))
		if k > 0:
			used.append(k)
	for i in range(1, 10):
		if not (i in used):
...
func _deduplicate_slot_keys() -> void:
	# Collect seen keys; for each duplicate, re-assign a fresh key
	var seen: Dictionary = {}  # key_int -> first wid that claimed it
	for wid: String in _weapon_slots:
		var k: int = int(_weapon_slots[wid].get("slot_key", 0))
		if k == 0:
			continue
		if seen.has(k):
			# Collision — give this weapon the next free key
			var new_key: int = _next_available_slot_key()
			_weapon_slots[wid]["slot_key"] = new_key
			if new_key > 0:
				seen[new_key] = wid
		else:
			seen[k] = wid
...
func _unlock_weapon_slot(weapon_id: String) -> void:
	...
	var wdef: Dictionary = _get_weapon_def(weapon_id)
	var assigned_key: int = int(wdef.get("slot_key", 0))
	if assigned_key == 0:
		assigned_key = _next_available_slot_key()
	_weapon_slots[weapon_id] = {
		"unlocked": true,
		"slot_key": assigned_key,
		"active_time": 0.0,
		"cooldown_time": 0.0,
	}
...
func get_active_weapons_display() -> Array[Dictionary]:
	...
	for wid: String in _weapon_slots:
		...
		var is_mode_w: bool = bool(wdef.get("is_mode", false))
		var mode_type: String = str(wdef.get("mode_type", ""))
		var sk: int = int(slot.get("slot_key", 0))
		var at: float = float(slot.get("active_time", 0.0))
		var is_mode_active: bool = is_mode_w and _active_mode == mode_type
		result.append({
			"name": str(wdef.get("name", wid)),
			"id": wid,
			"key": "[%d]" % sk if sk > 0 else "",
			"ready": cd <= 0.0 and at <= 0.0,
			"cooldown_time": cd,
			"cooldown_pct": cd / maxf(max_cd, 0.001),
			"active_time": at,
			"is_mode": is_mode_w,
			"mode_active": is_mode_active,
		})
	return result
