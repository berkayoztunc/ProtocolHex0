...
func _ready() -> void:
	...
	if heal_label.get_parent() == stats_row_1:
...
func show_notification(message: String, duration: float = 1.8) -> void:
	...
	panel_style.bg_color = Color(0.08, 0.05, 0.02, 0.92)
	panel_style.border_color = Color(0.7, 0.45, 0.12, 0.95)
	...
	label.add_theme_color_override("font_outline_color", Color(0.06, 0.03, 0.0, 0.9))
...
func _style_game_over_panel(hud_scale: float) -> void:
	_style_panel(game_over_panel, Color(0.06, 0.03, 0.02, 0.97), Color(0.7, 0.25, 0.1, 0.85))
	game_over_title.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(42.0, hud_scale, 1, 32.0)))
	game_over_title.add_theme_color_override("font_color", Color(1.0, 0.35, 0.2))
	game_over_title.add_theme_constant_override("outline_size", 3)
	game_over_title.add_theme_color_override("font_outline_color", Color(0.15, 0.04, 0.0, 0.95))
	for stat_lbl in [game_over_kills_label, game_over_level_label, game_over_weapon_label]:
		...
		stat_lbl.add_theme_color_override("font_color", Color(0.9, 0.75, 0.5))
		stat_lbl.add_theme_constant_override("outline_size", 1)
		stat_lbl.add_theme_color_override("font_outline_color", Color(0.06, 0.03, 0.0, 0.8))
...
func update_active_weapons(weapons_data: Array[Dictionary]) -> void:
	...
	for child in active_weapons_panel.get_children():
		child.queue_free()
	# Load weapon card frame textures
	var card_frame_tex: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/btn_frame_normal.png", 6, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	var card_frame_hover_tex: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/btn_frame_hover.png", 6, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	...
	for wdata in weapons_data:
		...
		# Pixel art frame for weapon cards
		if card_frame_tex != null:
			var frame: StyleBoxTexture
			if is_held and card_frame_hover_tex != null:
				frame = card_frame_hover_tex.duplicate()
			else:
				frame = card_frame_tex.duplicate()
			if not is_ready:
				frame.modulate_color = Color(0.5, 0.5, 0.5, 0.7)
		...
		if icon_tex != null:
			...
			# Scale icon to fill most of the card
		...
		# Weapon name label — small, below icon
		var lbl: Label = Label.new()
		var wname: String = str(wdata.get("name", ""))
		lbl.text = wname
		lbl.add_theme_font_size_override("font_size", maxi(label_font_size - 2, 9))
...
func _apply_ux_style() -> void:
	...
	modal_backdrop.color = Color(0.0, 0.0, 0.0, 0.85)

	# Stats background panel — pixel art HUD frame, minimal 9-slice
	var stats_tex: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/hud_stats_frame.png", 6, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	if stats_tex != null:
		stats_bg_panel.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
		stats_bg_panel.add_theme_stylebox_override("panel", stats_tex)
	else:
		var stats_style: StyleBoxFlat = StyleBoxFlat.new()
		stats_style.bg_color = Color(0.06, 0.04, 0.02, 0.92)
		stats_style.border_color = Color(0.6, 0.35, 0.1, 0.6)
		...
		stats_style.shadow_color = Color(0.3, 0.15, 0.0, 0.25)
	...
	# Level label — prominent orange
	level_label.add_theme_font_size_override("font_size", int(UiTextureUtils.scale_dimension(22.0, hud_scale, 1, 18.0)))
	level_label.add_theme_color_override("font_color", Color(1.0, 0.78, 0.35))
	level_label.add_theme_constant_override("outline_size", 3)
	level_label.add_theme_color_override("font_outline_color", Color(0.12, 0.06, 0.0, 0.9))

	# Compact stat labels — warm tones
	...
	for lbl in [kill_label, bomb_label, heal_label]:
		...
		lbl.add_theme_color_override("font_outline_color", Color(0.06, 0.03, 0.0, 0.9))
	kill_label.add_theme_color_override("font_color", Color(1.0, 0.82, 0.5))
	bomb_label.add_theme_color_override("font_color", Color(1.0, 0.55, 0.25))
	heal_label.add_theme_color_override("font_color", Color(0.55, 1.0, 0.45))

	# Info labels — weapon & targeting
	var info_font_size: int = int(UiTextureUtils.scale_dimension(13.0, hud_scale, 1, 11.0))
	for lbl in [weapon_label, targeting_label]:
		lbl.add_theme_font_size_override("font_size", info_font_size)
		lbl.add_theme_constant_override("outline_size", 1)
		lbl.add_theme_color_override("font_outline_color", Color(0.06, 0.03, 0.0, 0.8))
	weapon_label.add_theme_color_override("font_color", Color(0.85, 0.68, 0.4))
	targeting_label.add_theme_color_override("font_color", Color(0.7, 0.75, 0.9))

	# Bar sizes — thin bars, reasonable height
	var bar_scale: float = hud_scale * clampf(viewport_size.x / 1600.0, 0.85, 1.1)
	var health_bar_w: float = roundf(280.0 * bar_scale)
	var health_bar_h: float = roundf(26.0 * bar_scale)
	health_bar_w = maxf(health_bar_w, 220.0)
	health_bar_h = clampf(health_bar_h, 20.0, 32.0)
	var xp_bar_w: float = health_bar_w
	var xp_bar_h: float = clampf(roundf(14.0 * bar_scale), 10.0, 18.0)
	health_bar.custom_minimum_size = Vector2(health_bar_w, health_bar_h)
	xp_bar.custom_minimum_size = Vector2(xp_bar_w, xp_bar_h)

	_style_progress_bar(health_bar, Color(0.9, 0.3, 0.15, 1.0))
	_style_progress_bar(xp_bar, Color(0.85, 0.6, 0.15, 1.0))

	_style_panel(controls_panel, Color(0.06, 0.04, 0.02, 0.95), Color(0.55, 0.35, 0.12, 0.7))
	_style_panel(level_up_panel, Color(0.06, 0.04, 0.02, 0.97), Color(0.7, 0.42, 0.1, 0.85))
	_style_panel(confirm_menu_panel, Color(0.06, 0.04, 0.02, 0.97), Color(0.7, 0.42, 0.1, 0.85))
	...
	level_up_title.add_theme_color_override("font_color", Color(1.0, 0.78, 0.35))
	level_up_title.add_theme_constant_override("outline_size", 2)
	level_up_title.add_theme_color_override("font_outline_color", Color(0.12, 0.06, 0.0, 0.9))
...
func _layout_hud() -> void:
	...
	var weapon_row_h: float = UiTextureUtils.scale_dimension(15.0, hud_scale, 1, 12.0)
	# Full layout: level + health bar + xp bar + stats row 1 + stats row 2 + weapon + separators
	var stats_content_h: float = level_row_h + health_bar.custom_minimum_size.y + xp_bar.custom_minimum_size.y + stat_row_h * 2.0 + weapon_row_h + sep * 6.0
...
func _style_panel(panel: PanelContainer, bg: Color, border: Color) -> void:
	var textured: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/panel_frame_orange.png", 14, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	if textured != null:
	...
	style.shadow_color = Color(0.3, 0.15, 0.0, 0.35)
	style.shadow_size = 10
	panel.add_theme_stylebox_override("panel", style)


func _style_progress_bar(bar: ProgressBar, fill_color: Color) -> void:
	# Empty frame as background, solid fill on top — no 9-slice, full stretch
	var frame_path: String = "res://assets/ui/bars/bar_frame_empty.png"
	var fill_path: String = "res://assets/ui/bars/bar_fill_orange.png"

	var textured_bg: StyleBoxTexture = UiTextureUtils.load_stylebox_texture(frame_path, 0, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	var textured_fill: StyleBoxTexture = UiTextureUtils.load_stylebox_texture(fill_path, 0, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	if textured_bg != null and textured_fill != null:
		bar.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
		bar.add_theme_stylebox_override("background", textured_bg)
		# Tint the fill to match the bar's intended color
		textured_fill.modulate_color = fill_color
		bar.add_theme_stylebox_override("fill", textured_fill)
		return

	var background_style: StyleBoxFlat = StyleBoxFlat.new()
	background_style.bg_color = Color(0.08, 0.06, 0.03, 0.95)
	background_style.border_color = Color(0.4, 0.25, 0.1, 1.0)
...
func _style_button(button: Button) -> void:
	...
	if tex_normal != null and tex_hover != null:
		...
		var normal: StyleBoxFlat = StyleBoxFlat.new()
		normal.bg_color = Color(0.1, 0.07, 0.03, 1.0)
		...
		var hover: StyleBoxFlat = normal.duplicate()
		hover.bg_color = Color(0.16, 0.1, 0.04, 1.0)
		hover.border_color = Color(0.85, 0.55, 0.15, 1.0)
		hover.shadow_color = Color(0.6, 0.3, 0.0, 0.2)
		hover.shadow_size = 5
		var pressed: StyleBoxFlat = normal.duplicate()
		pressed.bg_color = Color(0.06, 0.04, 0.02, 1.0)
		pressed.border_color = Color(0.5, 0.32, 0.1, 1.0)
		var disabled: StyleBoxFlat = normal.duplicate()
		disabled.bg_color = Color(0.06, 0.05, 0.04, 0.85)
		disabled.border_color = Color(0.25, 0.18, 0.1, 0.7)
		button.add_theme_stylebox_override("normal", normal)
		button.add_theme_stylebox_override("hover", hover)
		button.add_theme_stylebox_override("pressed", pressed)
		button.add_theme_stylebox_override("disabled", disabled)
	button.add_theme_color_override("font_color", Color(0.92, 0.75, 0.42))
	button.add_theme_color_override("font_hover_color", Color(1.0, 0.85, 0.5))
	button.add_theme_color_override("font_pressed_color", Color(0.78, 0.6, 0.3))
	button.add_theme_color_override("font_disabled_color", Color(0.4, 0.32, 0.2))
...
func _style_utility_button(button: Button) -> void:
	...
	# Pixel art button frames for utility buttons
	var tex_normal: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/btn_frame_normal.png", 6, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	var tex_hover: StyleBoxTexture = UiTextureUtils.load_stylebox_texture("res://assets/ui/panels/btn_frame_hover.png", 6, StyleBoxTexture.AXIS_STRETCH_MODE_STRETCH)
	if tex_normal != null and tex_hover != null:
		button.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
		...
		normal.border_color = Color(0.45, 0.28, 0.1, 0.7)
		normal.border_width_left = 1
		normal.border_width_top = 1
		normal.border_width_right = 1
		normal.border_width_bottom = 1
		...
		var hover: StyleBoxFlat = normal.duplicate()
		hover.bg_color = Color(0.12, 0.09, 0.04, 0.92)
		hover.border_color = Color(0.75, 0.48, 0.12, 0.9)
		hover.shadow_color = Color(0.6, 0.3, 0.0, 0.15)
		hover.shadow_size = 4
		var pressed: StyleBoxFlat = normal.duplicate()
		pressed.bg_color = Color(0.04, 0.04, 0.06, 0.9)
		pressed.border_color = Color(0.55, 0.35, 0.1, 0.8)
		button.add_theme_stylebox_override("normal", normal)
		button.add_theme_stylebox_override("hover", hover)
		button.add_theme_stylebox_override("pressed", pressed)
	button.add_theme_color_override("font_color", Color(0.78, 0.62, 0.4))
	button.add_theme_color_override("font_hover_color", Color(1.0, 0.82, 0.45))
